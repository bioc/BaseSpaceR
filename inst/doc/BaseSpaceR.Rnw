...
it also facilitate the access to R's rich environment of statistical and data analysis
open source tools.
...
\item Portability works on most platforms, Linux, Windows, Mac OS X.
\end{itemize}
...
the sections to follow. It is essential for the developer to read through BaseSpace 
...
especially the \href{https://developer.basespace.illumina.com/docs/content/documentation/getting-started/overview#Steps_for_developing_your_first_App}{Steps for developing your first App} 
...
your system. We assume that the user have the \href{http://www.r-project.org}{R environment}
already installed and it is fairly familiar with it. R 2.15.0 or later and {\tt RCurl}
and {\tt RJSONIO} packages are required to be able to install and run {\tt BaseSpaceR}.
 
{\it The {\tt BaseSpaceR} package is available from the ...
...
a response in the form of a JSON object is returned.

Conceptually there are two response types, an individual response and a collection 
response. The former is used when querying an individual item/instance within a resource,
specified by the item ID, and it is modeled by the {\tt Item} class. The later is used for
listing the items/instances available for a given resource and it is modeled by the
{\tt Collection} class.

We briefly describe the {\tt Item} and {\tt Collection} interfaces.
...
within this document we are limited by the permission to the BaseSpace resources.
